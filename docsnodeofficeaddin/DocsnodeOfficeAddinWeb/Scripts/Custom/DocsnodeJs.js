...
BRDocsNodeJS.load = function () {
        ...
        try {
            ...
            callAjaxGet(url).done(function (data) {
                ...
                for (var i = 0; i < resultsCount; i++) {
                    ...
                    $.map(row.Cells.results, function (item, idx) {
                        if (item.Key === "Path") {
                            siteUrl = item.Value;
                        }
                        if (item.Key === "Title") {
                            siteTitle = item.Value;
                        }
                    });
        ...
        if (SPToken && !items) {
            ...
            $.ajax({
                ...
                if (!response.folderAccess) {
                    var Files = response.d.Files.results.filter(data => data.Name.lastIndexOf(".doc") > 1 || data.Name.lastIndexOf(".dot") > 1);
                    //for (var i = 0; i < response.d.Files.results.length; i++) {
                    //    var Files = data.Name.lastIndexOf(".docx") || data.Name.lastIndexOf(".dotx");
                    //}
    ...
    function bindingWebAllTemp(imageURLs, data, i) {
        ...
        if (data[i].Name.indexOf(docsNodeFilterExtention) != -1 ||
            data[i].Name.indexOf(docsNodeFilterExtention.toUpperCase()) != -1 || data[i].ContentType.Name === "Folder" || data[i].Name.toLowerCase().indexOf(".dotx") != -1 ) {
    ...
    function getMetaDataFromContentType(docContentTypeName, docId) {
        ...
        try {
            ...
            //removing guid string for client issue fix.. 
            var url = SPURL + templateServerRelURL + "/_api/web/lists('" + TemplateLibraryDisplayName + "')/contenttypes?$expand=Fields";
            callAjaxGet(url).done(function (data) {
