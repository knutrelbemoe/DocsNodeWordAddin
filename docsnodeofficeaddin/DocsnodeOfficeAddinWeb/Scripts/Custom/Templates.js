...
var currentTextPath = "";
var currentFolderTextSnippets = []
var IMGResultCount = 0;
var IMGFolderReset = 0;
// var ORG_ROOT_WEB_TEMPLATE = {};
// var ORG_TENANT_TEMPLATE = {};

// Added by Amartya for image and text snippet URL (03-Dec-2020) [Start]
...
// Added by Amartya for image and text snippet URL (03-Dec-2020)    [End]

(function () {
    "use strict";

    // The initialize function must be run each time a new page is loaded.
    Office.initialize = function (reason) {      // Later uncomment
        $(document).ready(function () {
            ...
            $("body").on("click", ".txtSnippet", function () {
                ...
                var snippetUrl = decodeURI($(this).data("fileurl"))
                snippetUrl = snippetUrl.replace(/#/g, "'");
                if (snippetUrl) {
                    LoadSnippetFileConent(snippetUrl + "/$value?binaryStringResponseBody=true").then(function (data) {
                        if (data.length > 0) {
                            insertParagraphs(data);
                        }
                    });
                }
            ...
            //            loadImagesOnScroll();
            //        }
            //        if ($('.list.SideContentBox.active').attr("id") == "one") // Text section
            //        {
            //            loadTextOnScroll();
            //        }
            //    }
            ...
            $('#txtSearch').on('keypress', function (e) {
                if (e.which == 13) {
                    ...
                    if ($(".SideContentBox.active").attr("id") == "one") {
                        ...
                        //var $_category = $("#hdnTextCategory").val();
                        if ($("#txtSearch").val() != "") {
                            searchTextSnippet($("#txtSearch").val().trim());
                        }
                        else {
                            clearSearchText();
                        }
                    ...
                    if ($(".SideContentBox.active").attr("id") == "two") {
                        ...
                        var $_category = $("#hdnCategory").val();
                        if ($("#txtSearch").val() != "") {
                            seachImages($("#txtSearch").val().trim());
                        }
                        else {
                            clearSearchText();
                        }
...
function GetConfigurations() {
    ...
    //     ORG_ROOT_WEB_TEMPLATE = JSON.parse(values);
    //     console.log("ORG_ROOT_WEB in Template JS:" + JSON.stringify(ORG_ROOT_WEB));
    // });

    // getLocalForageItem("OrgTenant").done(function (values) {
    //     ORG_TENANT_TEMPLATE = JSON.parse(values);
    //     console.log("ORG_TENANT in Template JS:" + JSON.stringify(ORG_TENANT));
                ...
                if (range.isEmpty) { //Check if the selection is empty    
                    ...
                    return context.sync().then(function () {
                        ...
                        if (txtLine.indexOf('<img') > -1 && localStorage.platform == 'PC') {
                            ...
                            $('#saveTextSnippetLocation').text(`Home${currentTextPath}`); // Added by Amartya on 10-01-2020 for Category
...
function addNewTextSnippet() {
    try {
        ...
        if (
            txtSnippetTitle != ''
            &&
            // Added by Amartya on 10-01-2020 for Checking Duplicate Text Snippet Name
            currentFolderTextSnippets.map(snippet => snippet.Name.toUpperCase().replace(/.TXT/, "")).indexOf(txtSnippetTitle.toUpperCase()) === -1) {
            ...
            // });
            // Added by Amartya on 10-01-2020 for Checking Duplicate Text Snippet Name
            var newTextSnippetPayload = {
                "SPOUrl": ORG_ROOT_WEB.webUrl,
                "tenant": ORG_TENANT.id,
                "TargetSite": TextLibSourceListPath,
                "FolderPath": currentTextPath,
                "LibraryName": SnippetLibraryName,
                "TextSnippet": $('#richTxtSnippet').val(),
                "SnippetTitle": txtSnippetTitle,
                "SnippetDescription": txtSnippetDescript
            }

            var duplicateTextSnippetURL =
                `${SPURL}${TextLibSourceListPath}/_api/web/getfilebyserverrelativeurl('${TextLibSourceListPath}/${SnippetLibraryName}${currentTextPath}/${txtSnippetTitle}.txt')`;

            $.ajax({
                url: duplicateTextSnippetURL,
                method: "GET",
                headers: { "Accept": "application/json; odata=verbose", 'Authorization': 'Bearer ' + SPToken },
                ...
                error: function (error) {
                    ...
                    $.ajax({
                        ...
                        data: JSON.stringify(newTextSnippetPayload),
...
function searchTextSnippet(searchParam, categoryId) {
    ...
    if (categoryId && categoryId != "" && categoryId != "0") {
        ...
        var url = SPURL + templateServerRelURL + "_api/web/lists(guid'" + TemplateLibraryDisplayName + "')/items?$select=FileRef,FileLeafRef&$filter=substringof('" + searchParam + "',FileRef)";
        ...
        LoadSnippetFileConent(url).then(function (data) {
            ...
            catContainer.addClass('boxOfTemplate');
            itemContainer = $("#ulCat");
            if (data.d.results.length > 0) {
                ...
                $.map(data.d.results, function (text, index) {
                    // var str = "<li item-type='File'>" +
                    //     "<a href='#' class='liTitle'  attr-category='" + text.UniqueId + "'>" +
                    //     "<span class='file-icon'><i class='ms-Icon ms-Icon--TextDocument' aria-hidden='true'></i></span>" +
                    //     "<span class='fileName' title='" + text.Name + "'>" + (text.Name.length > 20 ? text.Name.substr(0, 25) + "..." : text.Name) + "</span>" +
                    //     "</a >" +
                    //     "</li>";
                    var str = "";

                    var fileurl = SPURL + TextLibSourceListPath + "/_api/Web/GetFileByServerRelativePath(decodedurl=#" + text.FileRef + "#)";
                    var filename = text.FileLeafRef.substring(0, text.FileLeafRef.lastIndexOf('.')) || text.FileLeafRef;
                    if (text.FileLeafRef.indexOf(".txt") > -1) {

                        str = "<li contenttypename='file' title='" + (filename) +
                            "' class='checkboxToggler txtSnippet' data-fileurl='" + encodeURI(fileurl) + "'>" +
                            // "onClick=\"insertImage('" + imageURL + "')\">" +
                            "<div class='box-img'>" +
                            "<img src='/images/icons/txtdoc.png' class='docimgouterbox'>" +
                            "</div>" +
                            `<span style="word-break: break-all" title="${filename}">${filename}</span>` +
                            "</li>";
                    }
                    else if (text.FileLeafRef.indexOf(".json") > -1) {
                        str = "<li contenttypename='file' title='" + (filename) +
                            "' class='checkboxToggler txtSnippet' data-fileurl='" + encodeURI(fileurl) + "'>" +
                            // "onClick=\"insertImage('" + imageURL + "')\">" +
                            "<div class='box-img'>" +
                            "<img src='/images/icons/jsondoc.png' class='docimgouterbox'>" +
                            "</div>" +
                            `<span style="word-break: break-all" title="${filename}">${filename}</span>` +
                            "</li>";
                    }
                    else if (text.FileLeafRef.indexOf(".xml") > -1) {
                        str = "<li contenttypename='file' title='" + (filename) +
                            "' class='checkboxToggler txtSnippet' data-fileurl='" + encodeURI(fileurl) + "'>" +
                            // "onClick=\"insertImage('" + imageURL + "')\">" +
                            "<div class='box-img'>" +
                            "<img src='/images/icons/xmldoc.png' class='docimgouterbox'>" +
                            "</div>" +
                            `<span style="word-break: break-all" title="${filename}">${filename}</span>` +
                            "</li>";
                    }
                ...
                catContainer.append("<div style='margin-top: 100px;color: red;text-align: center;'><label>No Records Found</label></div>");
...
function loadTextSnippetAjax(filterCondition, catContainer, itemContainer, categoryId) {
    ...
    // Added By Amartya for Text Snippet ( 17 Dec, 2020 ) [End]
    var textBreadCrumbArray = currentTextPath.split("/");
    textBreadCrumbArray = textBreadCrumbArray[textBreadCrumbArray.length - 1] ?
        textBreadCrumbArray : textBreadCrumbArray.slice(0, 1);
    var textPathSpan = "";

    textBreadCrumbArray.map(function (eachFolder, index) {

        var folderPath = textBreadCrumbArray.slice(0, index + 1).join("/");
        folderPath = folderPath ? folderPath : "/";

        if (textBreadCrumbArray.length === 1) {

            textPathSpan += "<span path='" + folderPath + "' class='breadcrumb-span'>" +
                "Home" +
                "</span>";
        }
        else if (index < textBreadCrumbArray.length - 1) {

            textPathSpan += "<span path='" +
                folderPath +
    ...
    $("#one ul.categoryItems").append("<div class='breadcrumb-div'>" + textPathSpan + "</div>");

    try {
        $('#one ul.categoryItems .breadcrumb-span').each(function (index, item) {
            $(this).unbind('click');
            $(this).bind({
                click: function () {
                    openDialogSpinner();
                    currentTextPath = $(this).attr('path') === "/" ? "" : $(this).attr('path');
                    $("#one ul.categoryItems").html("");
                    loadTextSnippetAjax(filterCondition, catContainer, itemContainer, categoryId)
                },
            });
    ...
    new Promise(function (callback, reject) {
        ...
        // Added By Amartya for Text Snippet ( 15 Dec, 2020 ) [Start]
        loadImageAndTextFromLibrary(currentTextPath, "Text Snippet List").then(function (response) {
            if (response.Files.results.length == 0 && response.Folders.results.length == 0) {
                catContainer.append("<div style='margin: 5px;color: red;'><label>No Records Found..!!</label></div>");
            }
            else {
                // For Folders
                try {
                    response.Folders.results
                        ...
                        .map((folder, index) => {
                            // var str = "<li item-type='Folder'>" +
                            //     "<a href='#' class='liTitle' attr-category='" + folder.UniqueId + "'>" +
                            //     "<span class='file-icon'><i class='ms-Icon ms-Icon--OpenFolderHorizontal' aria-hidden='true'></i></span>" +
                            //     "<span class='fileName' title='" + folder.Name + "'>" + (folder.Name.length > 20 ? folder.Name.substr(0, 25) + "..." : folder.Name) + "</span>" +
                            //     "<span class='fileEnter-icon'><i class='ms-Icon ms-Icon--ChevronRightSmall' aria-hidden='true'></i></span>" +
                            //     "</a >" +
                            //     "</li>"
                            var str = "<li title='" + (folder.Name) + "' class='checkboxToggler' contenttypename='folder'>" +
                                "<div>" +
                ...
                // For Images
                try {
                    currentFolderTextSnippets = response.Files.results;

                    response.Files.results.forEach((text, index) => {
                        // var str = "<li item-type='File'>" +
                        //     "<a href='#' class='liTitle'  attr-category='" + text.UniqueId + "'>" +
                        //     "<span class='file-icon'><i class='ms-Icon ms-Icon--TextDocument' aria-hidden='true'></i></span>" +
                        //     "<span class='fileName' title='" + text.Name + "'>" + (text.Name.length > 20 ? text.Name.substr(0, 25) + "..." : text.Name) + "</span>" +
                        ...
                        //     "</li>";
                        var str = "";
                        var fileurl = text.__metadata.uri.replace(/'/g, "#");
                        var filename = text.Name.substring(0, text.Name.lastIndexOf('.')) || text.Name;
                        if (text.Name.indexOf(".txt") > -1) {
                        ...
                        else if (text.Name.indexOf(".json") > -1) {
                            str = "<li contenttypename='file' title='" + (filename) +
                                "' class='checkboxToggler txtSnippet' data-fileurl='" + encodeURI(fileurl) + "'>" +
                                // "onClick=\"insertImage('" + imageURL + "')\">" +
                                "<div class='box-img'>" +
                        ...
                        else if (text.Name.indexOf(".xml") > -1) {
                            str = "<li contenttypename='file' title='" + (filename) +
                                "' class='checkboxToggler txtSnippet' data-fileurl='" + encodeURI(fileurl) + "'>" +
                                // "onClick=\"insertImage('" + imageURL + "')\">" +
                                "<div class='box-img'>" +
                ...
                try {
                    $("#one ul.categoryItems li").each(function (index, item) {
                        if ($(this).attr('contenttypename') === "folder") {
                            $(this).unbind('click');
                            $(this).bind({
                                click: function () {
                                    openDialogSpinner();
                                    currentTextPath = currentTextPath.concat(`/${$(this).attr('title')}`);
                                    $("#one ul.categoryItems").html("");
                                    loadTextSnippetAjax(filterCondition, catContainer, itemContainer, categoryId)
        ...
        // Added By Amartya for Text Snippet ( 15 Dec, 2020 ) [End]
...
function LoadCategoryandItemsOnPageLoad() {
    ...
    if (imageListSite.toLowerCase() == '/sites/docsnodeadmin/' || imageListSite.toLowerCase() == '/sites/docsnodeadmin') {
        ...
        // Added by Amartya for image loading (03-Dec-2020) [Start]

        populateImageFolderView(currentImagePath, true);

        // .error(function (error) {
        //     console.error("Error while loading images from home page", error);
        // })

        // Added by Amartya for image loading (03-Dec-2020) [End]
...
function populateTextFolderView(path, isFolderClicked) {

    if (isFolderClicked) {
        currentTextPath += !path ? "/" : currentTextPath === "/" ? path : "/" + path;
    }
    else {
        currentTextPath = path;
    }

    var imageBreadCrumbArray = currentTextPath.split("/");
    imageBreadCrumbArray = imageBreadCrumbArray[imageBreadCrumbArray.length - 1] ?
        imageBreadCrumbArray : imageBreadCrumbArray.slice(0, 1);
    var imagePathSpan = "";
    ...
    loadImageAndTextFromLibrary(currentTextPath, "Text Library").then(function (response) {
        currentFolderTextSnippets = [];
        openDialogSpinner();
        var $_ul = $("#two ul.categoryItems");
        $_ul.html("");
        $_ul.append("<div class='breadcrumb-div'>" + imagePathSpan + "</div>");
        ...
        if (response.Files.results.length == 0 && response.Folders.results.length == 0) {
            ...
            try {
                response.Files.results.forEach((image, index) => {
                    var imageURL = "https://docsnode.sharepoint.com".concat(image.ServerRelativeUrl);
                    toDataURL(imageURL, function (dataUrl) {
                        var base64result = "data:image/png;base64, " + dataUrl;
                        $_ul.append("<li title='" + (image.Name) + "' class='liIteamImg' onClick=\"insertImage('" + imageURL + "')\"><a href='#'><div class='liInnerImage'><img src='" + base64result + "'>" +
                            "</div></a></li>");
                    });
                });
...
function openDialogSpinner() {
    if ($("#WaitDialogFirst").is(":visible")) {
        $("#WaitDialog").hide();
    }
    else
    {
